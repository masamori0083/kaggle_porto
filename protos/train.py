...
if __name__ == '__main__':
    ...
    for params in tqdm(list(ParameterGrid(all_params))):
        ...
        sc_logloss_2 = np.mean(list_logloss_score)
        sc_gini_2 = np.mean(list_gini_score)
        logger.info('logloss: {}, gini: {}'.format(sc_logloss_2, sc_gini_2))
        logger.info('current min score: {}, params: {}'.format(min_score, min_params))
        if min_score > sc_gini_2:
            min_score = sc_gini_2
            min_params = params
    ...
    logger.info('data load end {}'.format(x_test.shape))
